...
if [ -f "main.tf" ]; then
    ...
    printf "$SRC_WORKSPACE_URL"'\n'"$SRC_TOKEN"'\n' | databricks configure --token --profile azure-dr-src >/dev/null 2>&1
    printf "$TGT_WORKSPACE_URL"'\n'"$TGT_TOKEN"'\n' | databricks configure --token --profile azure-dr-tgt >/dev/null 2>&1
else
