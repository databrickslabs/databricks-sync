...
def test_cleanup(tests_path, it_conf, src_cluster_api: ClusterApi, tgt_cluster_api: ClusterApi, src_policy_service: PolicyService,
                 ...
                 src_secret_api: SecretApi, tgt_secret_api: SecretApi, src_cloud_name, tgt_cloud_name):
    ...
    # src, tgt = [c.strip() for c in  cloud.split(",")]

    src_cl_js = (fixture_path / f'{src_cloud_name}_clusters.json').absolute()
    tgt_cl_js = (fixture_path / f'{tgt_cloud_name}_clusters.json').absolute()
    src_ip_js = (fixture_path / f'{src_cloud_name}_instance_pools.json').absolute()
    tgt_ip_js = (fixture_path / f'{tgt_cloud_name}_instance_pools.json').absolute()
    src_job_js = (fixture_path / f'{src_cloud_name}_jobs.json').absolute()
    tgt_job_js = (fixture_path / f'{tgt_cloud_name}_jobs.json').absolute()
    ...
    with open(tgt_cl_js) as jsonfile:
        tgt_clusters_json = json.load(jsonfile)
    with open(src_ip_js) as jsonfile:
        src_pools_json = json.load(jsonfile)
    with open(tgt_ip_js) as jsonfile:
        tgt_pools_json = json.load(jsonfile)
    with open(src_job_js) as jsonfile:
        src_jobs_json = json.load(jsonfile)
    with open(tgt_job_js) as jsonfile:
        tgt_jobs_json = json.load(jsonfile)
    ...
    cleanup_workspace.delete_clusters(src_cluster_api, src_clusters_json)
    cleanup_workspace.delete_pools(src_pool_api, src_pools_json)
    cleanup_workspace.delete_jobs(src_job_api, src_jobs_json)
    ...
    cleanup_workspace.delete_clusters(tgt_cluster_api, tgt_clusters_json)
    cleanup_workspace.delete_pools(tgt_pool_api, tgt_pools_json)
    cleanup_workspace.delete_jobs(tgt_job_api, tgt_jobs_json)
...
@pytest.mark.dependency(depends=['test_cleanup'], scope="session")
def test_env(env, src_cloud_name, tgt_cloud_name):
    print(env)
    src_profile = env["source"]
    src_config = get_config_for_profile(src_profile)
    print(f"src_profile = {src_profile}")
    print("src_config.host", src_config.host)

    tgt_profile = env["target"]
    tgt_config = get_config_for_profile(tgt_profile)
    print(f"tgt_profile = {tgt_profile}")
    print("tgt_config.host", tgt_config.host)
    assert True
...
@pytest.mark.dependency(depends=['test_env'], scope="session")
def test_src_api_client(src_api_client: ApiClient):
    print(src_api_client.url)
    assert src_api_client.url is not None


@pytest.mark.run(depends=['test_src_api_client'],scope="session")
...
@pytest.mark.dependency(depends=['test_src_create_policy'], scope="session")
...
@pytest.mark.dependency(depends=['test_src_create_pool'], scope="session")
...
@pytest.mark.dependency(depends=['test_src_pools', 'test_src_policies', 'test_src_dbfs_files'], scope="session")
def test_src_create_cluster(src_create_cluster: ClusterApi):
    print(src_create_cluster)
    assert src_create_cluster is not None


@pytest.mark.dependency(depends=['test_src_create_cluster'], scope="session")
...
@pytest.mark.dependency(depends=['test_src_clusters'], scope="session")
def test_src_create_job(src_create_job):
    print(src_create_job)
    assert src_create_job is not None


@pytest.mark.dependency(depends=['test_src_create_job'], scope="session")
def test_src_jobs(src_job_api: JobsApi):
    print(src_job_api)
    assert src_job_api is not None


@pytest.mark.dependency(depends=['test_src_dbfs_files'], scope="session")
def test_src_upload_notebooks(src_upload_notebooks):
    assert src_upload_notebooks is not None


@pytest.mark.dependency(depends=['test_src_upload_notebooks'], scope="session")
def test_src_notebooks(src_notebooks):
    assert src_notebooks


@pytest.mark.dependency(depends=['test_src_api_client'], scope="session")
def test_src_create_group(src_create_group):
    assert src_create_group is not None


@pytest.mark.dependency(depends=['test_src_create_group'], scope="session")
...
@pytest.mark.dependency(depends=['test_src_create_secrets'], scope="session")
def test_src_secrete(src_secrets):
    assert src_secrets


@pytest.mark.dependency(depends=['test_src_secrete', 'test_src_group', 'test_src_notebooks', 'test_src_dbfs_files', 'test_src_jobs'], scope="session")
...
@pytest.mark.dependency(depends=['test_src_export_direct'], scope="session")
...
@pytest.mark.dependency(depends=['test_tgt_import_direct'], scope="session")
def test_tgt_import_no_change(tgt_api_config: DatabricksConfig, tgt_api_client: ApiClient,env, caplog, tgt_cloud_name):
    throws_exception=None
...
@pytest.mark.dependency(depends=['test_tgt_import_no_change'], scope="session")
...
@pytest.mark.dependency(depends=['test_tgt_objects_exist'], scope="session")
